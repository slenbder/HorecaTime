...
├── config.py                      ✅ загрузка .env, списки ID
├── app/
│   ├── logging_config.py          ✅
│   ├── bot/
│   │   ├── handlers/
...
│   │   │   └── ...остальные       ❌
│   │   ├── keyboards/
│   │   │   ├── common.py          ✅ отдел + позиции
│   │   │   └── admin.py           ❌
│   │   └── middlewares/
│   │       └── roles.py           ✅ импорт ID из config (не из auth.py!)
│   ├── services/
...
| `developer` | Служебные команды + алерты об ошибках |
...
**Этап 1 (в работе):** Главное меню — `setMyCommands` по ролям через `BotCommandScopeChat`
...
- Клавиатуры выбора отдела и позиции
