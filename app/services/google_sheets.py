...
class GoogleSheetsClient:
    ...
    def ensure_user_in_current_month_hours(self, telegram_id: int, custom_position: Optional[str] = None) -> bool:
        ...
        try:
            r = new_row
            # ПОДСТАВИТЬ(".";"," ) — нормализует старые значения с точкой ("12.5") к запятой
            # ("12,5"), которую ЗНАЧЕН() корректно читает в русской локали Google Sheets.
            # Новые значения write_shift уже пишет с запятой, ПОДСТАВИТЬ для них нейтрален.
