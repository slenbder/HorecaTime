...
- **Bug #11 Этап 3: Добавлен TTL 15 минут для FSM states (updated_at + APScheduler cleanup)**
  - Колонка `updated_at` добавлена в `fsm_storage` (CREATE TABLE + ALTER TABLE миграция в `models.py`)
  - `set_state()` и `set_data()` в `app/db/fsm_storage.py` обновляют `updated_at` при каждом вызове
  - Новый файл `app/scheduler/fsm_cleanup.py`: `cleanup_expired_fsm_states()` удаляет записи старше 15 мин
  - APScheduler джоба `fsm_cleanup` зарегистрирована в `main.py` (интервал 5 минут)
  - Обновлён `docs/FSM_STATES_MAP.md`: добавлена секция «Automatic Cleanup»
...
**Дата:** 2026-04-05 – 2026-04-09
**Ветка:** `main` (PR #55 смёрджен)

### Docker setup (PR #55)
- `Dockerfile` — multi-stage build (builder + runtime)
- `docker-compose.yml` — сервис bot + volume для SQLite (`data/`)
- `.dockerignore` — исключены credentials, .env, __pycache__, .venv
- `DB_PATH = "data/bot.db"` — SQLite в Docker volume (переживает перезапуск)
...
- **Hotfix Bug #6 (коммит `218385e`)** — замена legacy `"Су-шеф"` → `"Руководящий состав"` в auth.py
  - В базе и коде оставался устаревший строковый литерал `"Су-шеф"` после Phase 1 переименования
  - Исправлен в `process_approve` и смежных проверках

- **Fix Bug #10 (коммит `34f0d13`)** — `pending_custom_title` передаётся в `ensure_user_in_current_month_hours`
  - `custom_title` не пробрасывался при одобрении заявки — сотрудник добавлялся без должности
  - Исправлена передача параметра через `_pending_admins`

- **Fix /shift position source (коммит `e6a163c`)** — `cmd_shift` читает position из SQLite, не из Техлиста
  - Техлист хранит `custom_title` ("Тест Шеф") в колонке E для Руководящего состава
  - `cmd_shift` читал position из Техлиста → получал custom_title → не находил в SIMPLE_H_POSITIONS
  - Исправлено: `position = user_data.get("position", "")` из SQLite (всегда каноническое значение)
  - Добавлено `logger.info` в начало `cmd_shift` для диагностики
  - Тесты: 55/55 ✅

---

## Рефакторинг custom_title → custom_position + колонка H ✅

**Дата:** 2026-04-10
**Ветка:** `refactor/clean-custom-position` (PR #56, смёрджен в main)
**Тесты:** 55/55 ✅

### Часть 1 — Переименование + колонка H (`google_sheets.py`, `auth.py`)

- Добавлен `COL_CUSTOM_POSITION = 8  # H` в константы Техлиста
- `get_user_by_telegram_id` — возвращает `"custom_position"` из колонки H (`row[7]`)
- `add_or_update_pending_user` — новый параметр `custom_position`, пишет в колонку H
  - Новые строки: диапазон `A:H` (был `A:F`)
  - Обновление существующих: добавлен `{"range": f"H{row_idx}", ...}` в batch_update
- `ensure_user_in_current_month_hours` — `custom_title` → `custom_position` в сигнатуре и теле
- `auth.py` — все вхождения `custom_title` заменены на `custom_position` (переменные, строковые ключи FSM, ключи dict)

### Часть 2 — Разделение `position` и `custom_position` (`auth.py`)

- Убран хак `position_for_sheet`: ранее для Руководящего состава в E Техлиста писался `custom_title` вместо базовой позиции
- `add_or_update_pending_user` теперь всегда получает `position` (каноническое) → колонка E, `custom_position` → колонка H

### Часть 3 — Линейный approve flow (`auth.py`)

- Удалён reverse mapping (`_CANONICAL_POSITIONS`, проверка `raw_position not in _CANONICAL_POSITIONS`)
- Удалена FSM-ветка в `process_approve` — ранее при отсутствии `custom_title` в `_pending_admins` администратору предлагалось ввести должность повторно через Telegram
- Удалены `_KNOWN_POSITIONS` и `normalized_position`
- `position` и `custom_position` читаются напрямую из Техлиста (E и H)
- `process_approve`: ~120 строк → ~55 строк, один линейный путь для всех позиций

### Часть 4 — Документация

- `TECH_REFERENCE.md`: Техлист — E описан как «базовая позиция (всегда каноническая)», добавлена строка H(8) — Должность
- `CLAUDE.md`: терминология (`custom_title` → `custom_position`), Key Learnings обновлены (убраны 4 устаревших пункта про reverse mapping, добавлены 3 актуальных)
...
- **Должность** (custom_position, ранее `custom_title`) = "Шеф ЗЦ", "Бренд-шеф" (только для Руководящий состав и Грузчик/Закупщик)
- Техлист: E = базовая позиция (всегда каноническая), H = `custom_position`
...
## Этап 11: Audit Phase 2 — Важные баги (2026-04-05)

**Ветки:** `fix/phase-2-audit` + `fix/phase-2-bug11-fsm-cancel`  
**Цель:** Закрыть 11 важных багов из FINAL_AUDIT.md Phase 2  
**Результат:** ✅ Все баги закрыты, 18 новых тестов, готов к деплою

### Закрытые баги (11):

1. **Bug #9** — Email маскировка в логах (`mask_email()`)
2. **Bug #10** — Retry pattern для Google Sheets (3 метода: `mark_user_approved`, `get_sheet_id_by_name`, `get_section_range`)
3. **Bug #12** — `_parse_time` → `parse_time` (публичный API)
4. **Bug #13** — asyncio.Lock для `_mg_*` state (per-user locks по `mgid`)
5. **Bug #14** — `requests>=2.31.0` в requirements.txt
6. **Bug #15** — `.env.example` создан
7. **Bug #16** — Тесты для `user_rates` CRUD (8 тестов: get/set/history)
8. **Bug #17** — Тесты для `snapshot_user_rates_history()` (4 теста)
9. **Bug #18** — Тесты для Google Sheets методов (6 тестов)
10. **Bug #11 (3 этапа)** — FSM отмена и таймауты:
    - Этап 1: Reconnaissance — карта FSM states (`docs/FSM_STATES_MAP.md`)
    - Этап 2: `/cancel` команда (выход из FSM + edge case медиагруппы Официанта)
    - Этап 3: TTL 15 минут (APScheduler cleanup + `updated_at` в `fsm_storage`)
11. **Bug #19** — Удалён Pillow из requirements.txt

### Хотфиксы:
- Формулы S/AJ/AK обернулись в кавычки → `USER_ENTERED` для формул, `RAW` для user data
- `app/db/fsm_storage.py` добавлен в git (был исключён `.gitignore`)
...
### Технические детали:
- `mask_email()` вынесен в `app/utils/text_utils.py`
- `_mg_locks: dict[str, asyncio.Lock]` — защита от race condition
- FSM storage теперь с `updated_at` колонкой
- APScheduler джоба `fsm_cleanup` каждые 5 минут
- Retry pattern: `try → except → logger.error → _reconnect() → retry`

**Статус:** Готов к pre-deploy аудиту и деплою 🚀
