...
- Используется только для Руководящий состав (например "Шеф Иванов", "Су-шеф ЗЦ")
...
| H (8) | Должность | `custom_position` для РС/Грузчик/Закупщик (пусто для остальных позиций) |
...
Таблица для хранения FSM состояний (aiogram 3 custom storage).

| Колонка | Тип | Описание |
|---------|-----|----------|
| chat_id | INTEGER | ID чата |
| user_id | INTEGER | ID пользователя |
| state | TEXT | Текущее состояние FSM (например `ShiftStates.waiting_shift_input`) |
| data | TEXT | JSON с данными FSM |
| updated_at | TEXT | Timestamp последнего обновления (для TTL cleanup) |

**Primary key:** `(chat_id, user_id)`  
**TTL:** 15 минут (автоматическая очистка через APScheduler каждые 5 минут)  
**Storage:** `app/db/fsm_storage.py` — кастомная реализация `BaseStorage`
...
Константы **не в config.py** — определены локально в каждом handler-файле.

### userhours.py (cmd_shift логика)

```python
# Позиции кухни (включая вспомогательные)
KITCHEN_POSITIONS = {
...
**Важно:** `/shift` читает `position` из **SQLite** (`user_data.get("position")`),
а не из Техлиста. Техлист хранит `custom_title` в колонке E для Руководящего состава.

### superadmin.py / admin.py (ставки)

```python
_POSITIONS_WITH_EXTRA = {"Бармен", "Барбэк", "Раннер"}

_DEPT_POSITIONS_ORDER = {           # superadmin.py
...
waiting_kitchen_title   # Ввод custom_title для Руководящий состав
